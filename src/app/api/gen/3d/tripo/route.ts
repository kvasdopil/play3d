...
export async function POST(req: NextRequest) {
    try {
        ...
        console.log('Parsed request:', { imageDataUrl: imageDataUrl?.substring(0, 50) + '...', prompt });
        ...
        const stsResp = await fetch('https://api.tripo3d.ai/v2/openapi/upload/sts', {
            method: 'POST',
            headers: {
        ...
        const imageId =
            stsJson?.data?.image_token || stsJson?.data?.image_id || stsJson?.data?.id || stsJson?.data?.file_id;
        if (!imageId) {
        ...
        const fileType = blob.type === 'image/jpeg' ? 'jpg' :
