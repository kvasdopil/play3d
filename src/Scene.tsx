...
export function Scene() {
    ...
    useEffect(() => {
        ...
        controls.addEventListener('hoveron', onHoverOn);
        controls.addEventListener('hoveroff', onHoverOff);
        controls.addEventListener('mouseDown', onMouseDown);
        controls.addEventListener('mouseUp', onMouseUp);
        return () => {
            try {
                controls.removeEventListener('hoveron', onHoverOn);
                controls.removeEventListener('hoveroff', onHoverOff);
    ...
    return (
        <div className="w-screen h-screen relative">
            ...
            >
                    ...
                    sceneObjects.map((obj) => (
                        ...
                        >
                            <Model3D
                                ...
                                colorHex={renderMode === 'solid' ? pastelFromId(obj.id) : undefined}
                            />
                        </group>
                    ))
                ) : (
                ...
                <CursorWheelZoom controlsRef={orbitControlsRef} enabled={orbitEnabled} />
            </Canvas>
...
function CursorWheelZoom({ controlsRef, enabled }: { controlsRef: any; enabled: boolean }) {
    ...
    useEffect(() => {
        ...
        const onWheel = (e: WheelEvent) => {
            ...
            const maxDist = typeof controls.maxDistance === 'number' ? controls.maxDistance : Infinity;
            ...
            const viewPlane = new Plane().setFromNormalAndCoplanarPoint(camDir, target);

            const raycaster = raycasterRef.current;
            raycaster.setFromCamera(ndcRef.current, camera);
            const hitPoint = raycaster.ray.intersectPlane(viewPlane, pointRef.current);
            ...
            const newCam = point.clone().add(camPos.sub(point).multiplyScalar(factor));
            const newTarget = point.clone().add(target.sub(point).multiplyScalar(factor));
